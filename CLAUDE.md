# CLAUDE.md — esneiderbravo Agent Operating Rules

Agent rules for the **esneiderbravo** repository (Esneider Bravo's GitHub
profile README: the landing page rendered at github.com/esneiderbravo, with a
daily GitHub Actions workflow that regenerates the stats and top-language SVGs).
...
Trunk-based — commits land directly on `main` (history shows no feature
branches; optional short-lived `topic/<slug>` branches for larger changes).
Commits are imperative, sentence-case English, concise, with no
conventional-commit prefixes and no ticket IDs. Code reads like its neighbors;
comments carry constraints, never ticket IDs. Full rules:
