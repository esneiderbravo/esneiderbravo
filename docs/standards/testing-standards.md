# Testing & Quality Gates — esneiderbravo
...
- **Tests**: No test runner (no application code). The gate is: the README
  renders correctly on the GitHub profile, and the Metrics GitHub Action
  completes green (run it via `workflow_dispatch` after touching `metrics.yml`).
- **Lint / type-check**: No linter configured. Optional Markdown hygiene:
...
> **This is a content repo.** There is no unit/integration suite. "What must be
> tested" and "Test hygiene" below apply if application code is ever added; until
> then the real gates are the two above plus the manual verification that
> follows.
